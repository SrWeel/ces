...
INNER JOIN cesdb_aroriginal.dns_enfermeria AS e
       ON gi.enferm_enlace = e.enferm_enlace
INNER JOIN cesdb_aroriginal.dns_atencion AS a
       ON e.atenc_id = a.atenc_id
...
";
    $rs_ingesta = $DB_gogess->executec($sql_ingesta, array($clie_id));
...
INNER JOIN cesdb_aroriginal.dns_enfermeria AS e
       ON ge.enferm_enlace = e.enferm_enlace
INNER JOIN cesdb_aroriginal.dns_atencion AS a
       ON e.atenc_id = a.atenc_id
LEFT JOIN cesdb_arcombos.dns_eliminacion AS e_combo
       ON ge.geli_eliminacion = e_combo.eli_id
LEFT JOIN cesdb_aroriginal.app_usuario AS u 
       ON ge.usua_id = u.usua_id
WHERE a.clie_id = ?
ORDER BY ge.geli_fecharegistro DESC
";
    $rs_eliminacion = $DB_gogess->executec($sql_eliminacion, array($clie_id));
...
SELECT 
    ...
    -- Datos de enfermería y atención
    e.enferm_id,
    ate.atenc_id,
    ate.atenc_fecharegistro,
...
-- JOIN con dns_enfermeria
INNER JOIN cesdb_aroriginal.dns_enfermeria AS e
       ON ag.enferm_enlace = e.enferm_enlace

-- JOIN con dns_atencion
INNER JOIN cesdb_aroriginal.dns_atencion AS ate
       ON e.atenc_id = ate.atenc_id
...
";
    ...
    $rs_actividades = $DB_gogess->executec($sql_actividades, array($clie_id));
...
";
    ...
    $rs_dispositivos = $DB_gogess->executec($sql_dispositivos, array($clie_id));
    $sql_balance_hidrico = "
SELECT 
    gbh.gbh_id,
...
";
    ...
    $rs_balance_hidrico = $DB_gogess->executec($sql_balance_hidrico, array($clie_id));

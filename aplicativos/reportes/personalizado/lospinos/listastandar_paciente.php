...
{
   $sql3=" DATE_FORMAT(clie_registro,'%Y-%m-%d')>='".$fecha_i."' and DATE_FORMAT(clie_registro,'%Y-%m-%d')<='".$fecha_f."' and ";
}  
else
{
  ...
  if($fecha_i!='' and $fecha_f=='')
  {  
    $sql3=" DATE_FORMAT(clie_registro,'%Y-%m-%d')>='".$fecha_i."' and ";
  }
  else
  {
    if($fecha_i=='' and $fecha_f!='')
	{
	   $sql3=" DATE_FORMAT(clie_registro,'%Y-%m-%d')<='".$fecha_f."' and ";  
    }
  }
...
$lista_doc="select * from app_cliente left join cesdb_arcombos.faesa_tipoci on app_cliente.tipoci_id=cesdb_arcombos.faesa_tipoci.tipoci_id where ".$concatena_sql." order by clie_registro asc";
...
$lista_doc="select * from app_cliente left join cesdb_arcombos.faesa_tipoci on app_cliente.tipoci_id=cesdb_arcombos.faesa_tipoci.tipoci_id  order by clie_registro asc";
...
$documento.='<td >'.$rs_data->fields["clie_id"].'</td>';
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["tipoci_nombre"].'</td>';
...
$documento.='<td  style=mso-number-format:"@" >'.$rs_data->fields["clie_direccion"].'</td>';

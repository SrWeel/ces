...
if ($_SESSION['ces1313777_sessid_inicio']) {
  ...
  if ($clie_institucionval) {
    $sql4 = " 	conve_id ='" . $clie_institucionval . "' and ";
  }
...
<br>';
  ...
  if ($rs_data) {
    while (!$rs_data->EOF) {
      ...
      $tipodocd = $objformulario->replace_cmb("cesdb_arcombos.beko_tipocomprobante", "tipocmp_codigo,tipocmp_nombre", " where tipocmp_codigo like ", $rs_data->fields["tipocmp_codigo"], $DB_gogess);

      $estado_docval = '';
      $fonod_negaticos = '';
	  
	  $nconvenio='';
	  $nconvenio= $objformulario->replace_cmb("cesdb_arextension.dns_convenios", "conve_id,conve_nombre", " where conve_id =", $rs_data->fields["conve_id"], $DB_gogess);
	  
	  $tipocobro='';
	  $tipocobro= $objformulario->replace_cmb("cesdb_arextension.dns_tipoproceso", "tippo_id,tippo_nombre", " where tippo_id =", $rs_data->fields["tippo_id"], $DB_gogess);
	  ...
	  $documento .= '<td  nowrap="nowrap">' . $tipocobro . '</td>';
	   $documento .= '<td  nowrap="nowrap">' . $nconvenio . '</td>';
      ...
      $n_convenio = $objformulario->replace_cmb("cesdb_arextension.dns_convenios", "conve_id,conve_nombre", " where conve_id=", $rs_data->fields["conve_id"], $DB_gogess);

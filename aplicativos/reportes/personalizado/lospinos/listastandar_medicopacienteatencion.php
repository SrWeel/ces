...
<br>';
  ...
  if ($concatena_sql) {
    ...
    $lista_doc = "select * from app_cliente left join  cesdb_arextension.dns_tipoproceso on app_cliente.tippo_id=cesdb_arextension.dns_tipoproceso.tippo_id  left join cesdb_arextension.dns_convenios on app_cliente.conve_id=cesdb_arextension.dns_convenios.conve_id left join  lista_formulario on app_cliente.clie_id=lista_formulario.clie_id
	where  " . $concatena_sql . " order by clie_apellido asc";
  } else {

    $lista_doc = "select * from app_cliente left join  cesdb_arextension.dns_tipoproceso on app_cliente.tippo_id=cesdb_arextension.dns_tipoproceso.tippo_id  left join cesdb_arextension.dns_convenios on app_cliente.conve_id=cesdb_arextension.dns_convenios.conve_id left join lista_formulario on app_cliente.clie_id=lista_formulario.clie_id order by clie_apellido asc";
  }
  ...
  if ($rs_data) {
    while (!$rs_data->EOF) {
	 ...
	 // echo $prof_id;
	  ...
	  if($prof_id>0)
	  {
	  $n_especialidad = $objformulario->replace_cmb("cesdb_arextension.dns_profesion", "prof_id,prof_nombre", " where prof_id =",$prof_id, $DB_gogess);
	  }

...
$bu_kardex="select moviin_id,invp.centro_id,cen.centro_nombre AS bodega,sum(invp.moviin_totalenunidadconsumo) AS cantidad,invp.tipom_id AS tipom_id,invp.tipomov_id AS tipomov_id,invp.cuadrobm_id AS cuadrobm_id,invp.moviin_nlote AS moviin_nlote,invp.moviin_fechadecaducidad AS moviin_fechadecaducidad,centrorecibe_id,moviin_fecharegistro,invp.plantra_id,invp.plantrai_id,centroenvia_id,date_format(moviin_fecharegistro, '%Y-%m-%d') as solofecha,moviin_tbldispositivo,plantrai_id,precu_id,moviin_id,unid_id,uniddesg_id,moviin_preciocompra,compra_id from dns_movimientoinventario invp inner join dns_centrosalud cen on invp.centro_id=cen.centro_id where perioac_id='".$per_activo."' 
  ...
  $rs_listakardex = $DB_gogess->executec($bu_kardex);
                   if($rs_listakardex)
				   {
					...
					{	
						...
						$busca_facturac="select * from dns_compras where compra_id='".$rs_listakardex->fields["compra_id"]."'";
						$rs_facturac = $DB_gogess->executec($busca_facturac);
						
					?>
					 <tr>
						<td><?php echo $rs_listakardex->fields["moviin_fecharegistro"]." -> ".$rs_listakardex->fields["moviin_id"]; ?></td>
						<td><?php echo $rs_movnom->fields["tipomov_nombre"]." Compra: ID:".$rs_facturac->fields["compra_id"]." Factura:".$rs_facturac->fields["compra_nfactura"]; ?></td>
					<!--	<td></td>
						<td></td>  -->
...
Generado por: <?php 
echo $objformulario->replace_cmb("app_usuario","usua_id,usua_nombre,usua_apellido"," where usua_id=",@$_SESSION['ces1313777_sessid_inicio'],$DB_gogess);  ?>
<br />

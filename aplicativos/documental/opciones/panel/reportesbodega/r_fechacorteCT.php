...
//echo $bu_fcorte="select * from  dns_principalmovimientoinventario det left join dns_compras co on co.compra_id=det.compra_id inner join cmb_dns_cuadrobasicomedicamentos cm on cm.cuadrobm_id=det.cuadrobm_id where compra_fechaaprobacion<='".$fechaf."' and year(compra_fechaaprobacion)>='".$per_activo."' and cm.categ_id in (3,4,5,6,7,8) and det.tipomov_id in (2,17,8,9,10,16) order by nombre_med asc";
...
<div align="center">
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
      <td><div align="center"><strong>CL&Iacute,NICA DE ESPECIALIDADES DEL SUR </strong></div></td>
    </tr>
    <tr>
      <td><div align="center"><strong>Bodega: BODEGA PRINCIPAL Fecha de Corte: </strong><?php echo $fechai; ?></div></td>
    </tr>
    <tr>
...
//echo $lista_partida="select distinct cuadrobm_partidapresupuest from dns_compras co inner join dns_principalmovimientoinventario det on co.compra_id=det.compra_id inner join cmb_dns_cuadrobasicomedicamentos cm on cm.cuadrobm_id=det.cuadrobm_id where compra_fechaaprobacion<='".$fechai."' and year(compra_fechaaprobacion)>='".$per_activo."' and cm.categ_id in (3,4,5,6,7,8) and det.tipomov_id in (17,8,9,10,16) order by nombre_med asc";
...
//echo $bu_fcorte="select distinct cm.cuadrobm_id,moviin_nlote,moviin_fechadecaducidad,moviin_preciocompra,cuadrobm_partidapresupuest from  dns_principalmovimientoinventario det left join dns_compras co on co.compra_id=det.compra_id inner join cmb_dns_cuadrobasicomedicamentos cm on cm.cuadrobm_id=det.cuadrobm_id where (compra_fechaaprobacion<='".$fechaf."' or moviin_fecharegistro<='".$fechaf."') and (year(compra_fechaaprobacion)>='".$per_activo."' or perioac_id >='".$per_activo."') and cm.categ_id in (3,4,5,6,7,8) and det.tipomov_id in (2,17,8,9,10,16) and cuadrobm_partidapresupuest='".$rs_partida->fields["cuadrobm_partidapresupuest"]."' order by nombre_med asc";

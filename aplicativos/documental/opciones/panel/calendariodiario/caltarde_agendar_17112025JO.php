						...
						<tr>
							...
							if ($rs_personal) {
								while (!$rs_personal->EOF) {
									...
									// $lista_buscat="select terap_id,atenc_hc,especi_id,faesa_terapiasregistro.usua_id,faesa_terapiasregistro.clie_id,terap_fecha,terap_hora,terap_autorizacion,terap_estado,terap_fechapago,terap_nfactura,faesa_terapiasregistro.centro_id,faesa_terapiasregistro.usuar_id,terap_fecharegistro,terap_recuperacion,terap_observacion,terap_tipoevatera,tipopac_id,clie_nombre,clie_apellido,terap_motivo,terap_asiste,quiro_id,terap_medicompanies,terap_copago,conve.conve_nombre from  faesa_terapiasregistro left join app_cliente on faesa_terapiasregistro.clie_id=app_cliente.clie_id left join cesdb_arextension.dns_convenios conve on faesa_terapiasregistro.conve_id=conve.conve_id where faesa_terapiasregistro.usua_id=".$rs_personal->fields["usua_id"]." and terap_fecha='".$fecha_valor."' and terap_hora='".$arreglo_horas[$hi]."' and terap_cancelado=0";
									...
									if ($rs_lbuscat) {
										while (!$rs_lbuscat->EOF) {
											...
											if (@$rs_lbuscat->fields["tipocon_id"]) {
												...
												$n_tipoconsulta = "<br>" . $rs_btipoconsulta->fields["tipocon_nombre"];
											}
											...
											if ($rs_lbuscat->fields["terap_asiste"] == 1) {
												...
												$check_confirmado = '<input type="button" name="Submit" value="CONFIRMACION" onclick="' . $function_confirma . '" >';
									  ...
									  <tr>
										...
										<td ><center><span style="color:#000000" ><b>' . $n_seguro . "<BR>" . $paciente_data . $terap_motivo . $n_quirofano . $terap_medicompanies . $terap_copago . $n_tipoconsulta . '/</b></span></center></td>
										<td ></td>
									  </tr>
								 </table>';
											} else {
									  ...
									  <tr>
											<td>Estado: ' . $check_confirmado . '</td>
											<td>											
											<div id="estado_conf' . $rs_lbuscat->fields["terap_id"] . '" >' . $estado_confirmado . '</div></td>
											<td></td>
									  </tr>
								 </table>';
											}

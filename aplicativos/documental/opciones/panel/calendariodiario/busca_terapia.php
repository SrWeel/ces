...
//busca horario
    ...
    $hora_normalizada = substr($hora, 0, 5); // Tomar solo HH:MM
    ...
    if($rs_databh)
    {
        while (!$rs_databh->EOF) {
            ...
            if($dia_id==$ndia)
            {
                ...
                $horario_hora_bd = substr($rs_databh->fields["horario_hora"], 0, 5);
                $horario_horafin_bd = substr($rs_databh->fields["horario_horafin"], 0, 5);

                // Convertir a timestamps para comparación
                $hora_seleccionada_ts = strtotime("2025-01-01 " . $hora_normalizada);
                $hora_inicio_ts = strtotime("2025-01-01 " . $horario_hora_bd);
                $hora_fin_ts = strtotime("2025-01-01 " . $horario_horafin_bd);

                // Verificar si la hora seleccionada está dentro del rango
                if($hora_seleccionada_ts >= $hora_inicio_ts && $hora_seleccionada_ts <= $hora_fin_ts)
                {
                    $bandera_hora=1;
                    break; // Ya encontramos un horario válido, no seguir buscando
                }

...
if (@$_SESSION['ces1313777_sessid_inicio']) {
	...
	$lista_hijos = "select distinct clie_nombre,clie_apellido,app_cliente.clie_id,clie_rucci from app_cliente where clie_rucci  like '%" . trim($_POST["busca_paciente"]) . "%' or clie_nombre like '%" . trim($_POST["busca_paciente"]) . "%' or clie_apellido like '%" . trim($_POST["busca_paciente"]) . "%'";
...
';
	if ($rs_datahijos) {
		...
		while (!$rs_datahijos->EOF) {
			...
			$comilla_simple = "'";
			echo '<tr>
	<td><input name="radio_hc" type="radio" value="' . $rs_datahijos->fields["clie_rucci"] . "|" . $rs_datahijos->fields["clie_id"] . '" /></td>	
    <td nowrap="nowrap" style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:10px; cursor:pointer" >' . $rs_datahijos->fields["clie_rucci"] . '</td>

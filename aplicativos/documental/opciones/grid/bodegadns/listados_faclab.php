...
?></center>		  
   ...
   if($atc_val)
   {
		 ...
		 $sql_cp=" and dns_compras.compra_id in (".$busca_compras.") ";
		          ...
		          if($sqlconcatenado)
				   { 
				   $lista_campor="select * from dns_compras left join app_proveedor on dns_compras.proveevar_id=app_proveedor.provee_id where ".$sqlconcatenado." and  compra_laborat=1 and  dns_compras.emp_id='".@$_SESSION['ces1313777_sessid_emp_id']."' and compra_tipo=0 and tipom_id=1 and tipomov_id=17 and tipdoc_id not in (8,9,10) and compra_parainv=1 ".$sql_cp." order by  compra_id desc limit 200";	
				   
				   }
				   else
				   {
				    $lista_campor="select * from dns_compras left join app_proveedor on dns_compras.proveevar_id=app_proveedor.provee_id where   compra_laborat=1 and  dns_compras.emp_id='".@$_SESSION['ces1313777_sessid_emp_id']."' and compra_tipo=0 and tipom_id=1 and tipomov_id=17 and tipdoc_id not in (8,9,10) and compra_parainv=1 ".$sql_cp." order by  compra_id desc limit 200";

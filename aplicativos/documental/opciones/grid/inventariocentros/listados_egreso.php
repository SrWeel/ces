...
$centro_activoentrecentros=$objformulario->replace_cmb("dns_centrosalud","centro_id,centro_activoentrecentros"," where centro_id=",$_SESSION['ces1313777_centro_id'],$DB_gogess);

$centro_disposentrecentros=$objformulario->replace_cmb("dns_centrosalud","centro_id,centro_disposentrecentros"," where centro_id=",$_SESSION['ces1313777_centro_id'],$DB_gogess);
  ...
  $busca_compras='';
   ...
   if($atc_val)
   {
		 ...
		 $sql_cp=" and dns_invegresosvarios.egrec_id in (".$busca_compras.") ";
				   ...
				   //$lista_campor="select * from dns_invegresosvarios where categ_id='".$_POST["insu"]."' and  dns_compras.emp_id='".@$_SESSION['ces1313777_sessid_emp_id']."'  order by  compra_fecha desc";	  
				   if($sqlconcatenado)
				   { 
				   $lista_campor="select * from dns_invegresosvarios left join dns_centrosalud on  dns_invegresosvarios.centrod_id=dns_centrosalud.centro_id where ".$sqlconcatenado." and dns_invegresosvarios.emp_id='".@$_SESSION['ces1313777_sessid_emp_id']."' and tipom_id=2  and egrec_tipo=1 ".$sql2." ".$sql_cp." order by  egrec_id desc";
				   }
				   else
				   {
				   $lista_campor="select * from dns_invegresosvarios left join dns_centrosalud on  dns_invegresosvarios.centrod_id=dns_centrosalud.centro_id where  dns_invegresosvarios.emp_id='".@$_SESSION['ces1313777_sessid_emp_id']."' and tipom_id=2 and egrec_tipo=1 ".$sql2." ".$sql_cp." order by  egrec_id desc";
				   }

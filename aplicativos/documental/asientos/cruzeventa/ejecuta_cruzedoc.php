...
if ($_SESSION['ces1313777_sessid_inicio']) {
	...
	$busca_listabrrado = "select crudet_id from lpin_crucedocumentos inner join cesdb_arextension.lpin_crucedetalle on lpin_crucedocumentos.crudoc_enlace=cesdb_arextension.lpin_crucedetalle.crudoc_enlace where crudoc_id='" . $crudoc_id . "'";
	...
	$busca_cobros = "select * from lpin_crucedocumentos inner join cesdb_arextension.lpin_crucedetalle on lpin_crucedocumentos.crudoc_enlace=cesdb_arextension.lpin_crucedetalle.crudoc_enlace inner join lpin_ttransac on lpin_crucedocumentos.crudoc_tipotransaccion=lpin_ttransac.ttra_id where crudoc_id='" . $crudoc_id . "'";
			...
			if ($num_detasiento > 0) {
				...
				if ($rs_bcabecera->fields["comcont_id"] > 0) {
					...
					for ($i = 0; $i < count($array_haber); $i++) {
						...
						$lista_data = "INSERT INTO lpin_detallecomprobantecontable (detcc_id, detcc_cuentacontable, detcc_descricpion, detcc_referencia, detcc_entidad, detcc_debe, detcc_haber, usua_id, detcc_fecharegistro,comcont_enlace,detcc_tipo) VALUES (NULL, '" . $detcc_cuentacontable . "', '" . $detcc_descricpion . "', '" . $detcc_referencia . "', '','" . round($detcc_debe, 2) . "','" . round($detcc_haber, 2) . "','" . $_SESSION['ces1313777_sessid_inicio'] . "', '" . date("Y-m-d H:i:s") . "','" . $comcont_enlace . "','" . $detcc_tipo . "') ";
...
( '7', '" . $crudoc_fechaemision . "', '" . $concepto . "', '" . $doccab_ndocumento . "', 'APROBADO', 0, '" . $comcont_enlace . "', '" . $_SESSION['ces1313777_sessid_inicio'] . "', '" . $fecha_hoy . "','" . $_SESSION['ces1313777_centro_id'] . "', '" . $tabla_asiento . "', '" . $valor_id . "','AUTOMATICO','" . $doccab_anulado . "','beko_documentocabecera','" . $rs_bcobros->fields["crudoc_ndocumentoventa"] . "','" . $crudet_id . "');";

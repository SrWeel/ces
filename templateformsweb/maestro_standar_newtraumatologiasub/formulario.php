...
<?php
// ============================================
// CÓDIGO PHP PARA CARGAR DIAGNÓSTICOS - TRAUMATOLOGÍA
// ============================================
...
{
    // Obtener configuración de la tabla hija de diagnósticos de traumatología
    $busca_campodiag = "SELECT * FROM gogess_sisfield 
                        WHERE tab_name='dns_newtraumatologiaanamesis' 
                        AND ttbl_id=1 
    ...
    if($rs_campodiag && !$rs_campodiag->EOF)
    {
        ...
        // Buscar registro de traumatología
        $busca_trauma = "SELECT anam_enlace
                        FROM dns_newtraumatologiaanamesis
                        WHERE atenc_id = ".$atenc_id."
                        ORDER BY anam_id DESC
                        LIMIT 1";

        $rs_trauma = $DB_gogess->executec($busca_trauma, array());

        if($rs_trauma && !$rs_trauma->EOF && $rs_trauma->fields["anam_enlace"])
        {
            $anam_enlace = $rs_trauma->fields["anam_enlace"];

...
?>
fie_id:'<?php echo $this->fie_id;  ?>',
sess_id:'<?php echo $_SESSION['ces1313777_sessid_inicio']; ?>'
...
?>
<br>
<button type="button" class="mb-sm btn btn-primary"  onClick="genera_cafeproducto()"  style="background-color:#000066" > PRODUCTO CAFETERIA</button>  &nbsp;&nbsp;&nbsp;
<button type="button" class="mb-sm btn btn-primary"  onClick="genera_cafeempaque()"  style="background-color:#000066" > EMPAQUES CAFETERIA</button>
<br>
<?php
}
?>
...
$rs_lcanp = $DB_gogess->executec($lista_campos,array());
 if($rs_lcanp)
 {
	  ...
	  {
		  echo '<div id="divn_'.$rs_lcanp->fields["gridfield_nameid"].'" class="col-md-'.$rs_lcanp->fields["gridfield_tamano"].'"><label><b>'.$rs_lcanp->fields["gridfield_title"].'</b></label>';
...
function genera_cafeproducto()
{
$('#docdet_codprincipalx').val('CAF001');
llenar_dataproducto();
calcula_data();

}

function genera_cafeempaque()
{
$('#docdet_codprincipalx').val('EMPAC001');
llenar_dataproducto();
calcula_data();
...
echo " $('#divn_porcevr_idx').hide();
$('#divn_porcevi_idx').hide();
";

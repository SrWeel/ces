...
<?php
			...
			$buscadt="select * from ".@$tabla_llega." where ";
			
			$objformulario->sendvar["imgag_resumenclinicox"]='';
            $objformulario->sendvar["imgag_motivox"]='';
            
           // echo $objformulario->sendvar["imgag_tablaexternox"];
			...
			if($objformulario->sendvar["imgag_idexternox"]>0)
{
	...
	if($objformulario->sendvar["imgag_tablaexternox"]=='dns_newtraumatologiaanamesis')
	{
//	
//	imgag_resumenclinico

        $objformulario->sendvar["imgag_resumenclinicox"]=$rs_budiag->fields["anam_emferpactual"];
        $objformulario->sendvar["imgag_motivox"]=$rs_budiag->fields["anam_motivoconsulta"];
	}
	
	if($objformulario->sendvar["imgag_tablaexternox"]=='dns_newconsultaexternaanamesis')
	{
//	
//	imgag_resumenclinico
        ...
        $objformulario->sendvar["imgag_motivox"]=$rs_budiag->fields["anam_motivoconsulta"];
			...
			$datos_examenfisicoregional="select tb.gexamfis_observacion as Resumen_Clinico FROM cesdb_aroriginal.dns_newtraumatologiaanamesis aet LEFT JOIN cesdb_arextension.dns_newtraumatologiagridexamenfisico tb on aet.anam_enlace=tb.anam_enlace where aet.clie_id=".$clie_id." and tb.tiporg_nombre='CP'";
